...
%   Therefore literals in $c_1\tau\sigma$.
% Take a literal $C=C'\tau\sigma$ from $(c_1\setminus\{A\})\tau\sigma$.
% If $B$ is ground, that is $B=\lnot R(\bar{x}^{\bar{\alpha'}}_{{1..m+s}}())$,
% then $C$ is also ground from the definition of $\sigma$. Otherwise, 
% if $B$ is non-ground

% From the codomain of $\sigma$ we know that $B$ contains all variables of $C$.
% Also $B$ contains all variables of $(c_1\setminus\{A\})\tau\sigma$.
...
% The literal $A$ is a maximal literal in $c_1$, so $A\tau$ is a maximal literal in $c_1\tau$.
